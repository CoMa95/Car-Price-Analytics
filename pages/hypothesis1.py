...
def show_price_metrics(df):
    ...
    col2.metric(
        ...
        delta=f"{((diesel_avg - overall_avg) / overall_avg) * 100:+.1f}% vs overall"
    )
    col3.metric(
        label="Average Petrol Price (£)",
        value=f"{petrol_avg:,.0f}",
        delta=f"{((petrol_avg - overall_avg) / overall_avg) * 100:+.1f}% vs overall"
    )
...
def show_interpretation(df) -> None:
    ...
    pearson_corr, pearson_pval, spearman_corr, spearman_pval = calculate_correlation(df)
    t_stat, t_pval, u_stat, u_pval = run_statistical_tests(df)
    ...
    if (t_pval < 0.05 or u_pval < 0.05):
        st.success("""
        **Summary:**
        - The p-values for both the t-test and Mann–Whitney U test are below 0.05,
          indicating a **statistically significant difference** in car prices
        ...
        - Correlation results show that fuel type is related to price variation,
          suggesting manufacturers may price diesel and petrol cars differently
          due to performance, efficiency, or market demand.
        """)
    else:
        st.info("""
        **Summary:**
        - The p-values for both the t-test and Mann–Whitney U test are above 0.05,
          meaning there is **no statistically significant difference** in prices
          between petrol and diesel cars.
        - Therefore, we **cannot reject the null hypothesis (H₀)** —
          fuel type does not appear to significantly influence price in this dataset.
        """)
